...
const getSummaryEntry = (relation: SummaryRelation) => {
  return relation.summary_table?.[0] ?? null;
};
...
const getLatestSummary = (relations: SummaryRelation[] | null) => {
  ...
  return relations
    .filter((relation) => getSummaryEntry(relation))
    .sort((a, b) => {
      const first = new Date(getSummaryEntry(a)?.created_at ?? 0).getTime();
      const second = new Date(getSummaryEntry(b)?.created_at ?? 0).getTime();
      return second - first;
    })[0];
};
    ...
    <Stack gap="md">
      ...
      <Stack gap="md">
        {sortedRows.map((row) => {
          ...
          const summaryEntry = latestSummary ? getSummaryEntry(latestSummary) : null;
          const summaryText = summaryEntry?.summary ?? "Summary unavailable.";
